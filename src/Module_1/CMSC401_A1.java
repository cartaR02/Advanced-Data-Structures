...
public class CMSC401_A1 {
    ...
    public static void main(String[] args) {
        ...
        int[] arrayNumbers = new int[lineNumber];

        for (int i = 0; i < lineNumber; i++){
            int nextCandidate = scanner.nextInt();
            arrayNumbers[i] = nextCandidate;
        ...
        // clears counters to get real counts
        // current counts dont matter as they are just trying to 
        // get candidate
        candidatesArrayCounter[0] = 0;
        candidatesArrayCounter[1] = 0;
        candidatesArrayCounter[2] = 0;

        // running through array again to get actual counts 
        // from candidates
        for (int i = 0; i < lineNumber; i++){
            int currentNumberCheck = arrayNumbers[i];
            if (currentNumberCheck == candidatesArray[0]){
                candidatesArrayCounter[0]++;
            } else if (currentNumberCheck == candidatesArray[1]){
                candidatesArrayCounter[1]++;
            } else if (currentNumberCheck == candidatesArray[2]){
                candidatesArrayCounter[2]++;
            }
        }
